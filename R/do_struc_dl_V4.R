...
device <- "cpu" #if (cuda_is_available()) torch_device("cuda") else torch_device("cpu")
...
dimD_pref  <- ncol(D_pref)      # number of non-base models
dimD_cost  <- ncol(D_cost)      # number of all models (base + non-base)
...
################################################################################
## 3. Hyperparameters, activations, regularization, schedulers, and backbone
################################################################################
...
################################################################################
## 3B. Regularization and LR schedulers
################################################################################

# L2 regularization (weight decay) for each network
theta_weight_decay    <- 0.0   # e.g. 1e-4
hessian_weight_decay  <- 0.0
cost_weight_decay     <- 0.0
prop_weight_decay     <- 0.0

# L1 regularization for each network
theta_l1_lambda   <- 0.0       # e.g. 1e-6
hessian_l1_lambda <- 0.0
cost_l1_lambda    <- 0.0
prop_l1_lambda    <- 0.0

# Learning rate schedulers
# options: "none", "step", "cosine", "plateau"
theta_scheduler_type   <- "none"
hessian_scheduler_type <- "none"
cost_scheduler_type    <- "none"
prop_scheduler_type    <- "none"

# Scheduler specific parameters (adjust as needed)
...
################################################################################
## Helpers: L1 penalty and LR schedulers
################################################################################
...
# Simple helper to create schedulers
# This assumes the torch package has lr_step, lr_cosine_annealing, lr_reduce_on_plateau.
# If names differ in your torch version, adjust here once.
...
for (s in seq_len(Nsplits)) {
  ...
  optim_theta <- optim_adamw(
    theta_model$parameters,
    lr           = theta_lr,
    weight_decay = theta_weight_decay
  )
  
  theta_scheduler <- make_scheduler(
    optim_theta,
    theta_scheduler_type,
    theta_scheduler_params,
    num_epochs = theta_num_epochs
  )
  ...
  for (epoch in seq_len(theta_num_epochs)) {
    ...
    coro::loop(for (batch in train_dl_theta) {
      ...
      base_loss <- nnf_binary_cross_entropy_with_logits(
    ...
    if (!is.null(theta_scheduler)) {
      if (theta_scheduler_type == "plateau") {
        theta_scheduler$step(val_loss)
      } else {
  ...
  optim_H <- optim_adam(
    hessian_model$parameters,
    lr           = hessian_lr,
    weight_decay = hessian_weight_decay
  )
  
  hessian_scheduler <- make_scheduler(
    optim_H,
    hessian_scheduler_type,
    hessian_scheduler_params,
    num_epochs = hessian_num_epochs
  )
  ...
  for (epoch in seq_len(hessian_num_epochs)) {
    ...
    coro::loop(for (batch in train_dl_H) {
      ...
      base_loss_H <- nnf_mse_loss(H_pred_b, H_b)
      
      loss_H <- add_l1_penalty(
        loss       = base_loss_H,
        model      = hessian_model,
        lambda_l1  = hessian_l1_lambda,
        device     = device
      )
    ...
    if (!is.null(hessian_scheduler)) {
      if (hessian_scheduler_type == "plateau") {
        hessian_scheduler$step(val_loss_H)
      } else {
        hessian_scheduler$step()
      }
    }
  ...
  optim_cost <- optim_adam(
    cost_model$parameters,
    lr           = cost_lr,
    weight_decay = cost_weight_decay
  )
  
  cost_scheduler <- make_scheduler(
    optim_cost,
    cost_scheduler_type,
    cost_scheduler_params,
    num_epochs = cost_num_epochs
  )
  ...
  for (epoch in seq_len(cost_num_epochs)) {
    ...
    coro::loop(for (batch in train_dl_cost) {
      ...
      base_loss_b  <- 0.5 * (res_b^2)$mean()
      
      loss_b <- add_l1_penalty(
        loss       = base_loss_b,
        model      = cost_model,
        lambda_l1  = cost_l1_lambda,
        device     = device
      )
    ...
    if (!is.null(cost_scheduler)) {
      if (cost_scheduler_type == "plateau") {
        cost_scheduler$step(val_loss)
      } else {
        cost_scheduler$step()
      }
    }
  ...
  optim_prop <- optim_adam(
    prop_model$parameters,
    lr           = prop_lr,
    weight_decay = prop_weight_decay
  )
  
  prop_scheduler <- make_scheduler(
    optim_prop,
    prop_scheduler_type,
    prop_scheduler_params,
    num_epochs = prop_num_epochs
  )
  ...
  for (epoch in seq_len(prop_num_epochs)) {
    ...
    coro::loop(for (batch in train_dl_prop) {
      ...
      base_loss_b <- nnf_binary_cross_entropy_with_logits(
    ...
    if (!is.null(prop_scheduler)) {
      if (prop_scheduler_type == "plateau") {
        prop_scheduler$step(val_loss)
      } else {
        prop_scheduler$step()
      }
    }
...
H_inv_all = linalg_pinv(H_hat_all_clamp + torch_tensor(diag(1e-3, nrow = dimD, ncol = dimD)))
...
plot_token_bins <- function(y_vec, main_title) {
  ...
  p_obj <- binsreg::binsreg(
    ...
    simsgrid   = 100,
    randcut    = 1, 
    cluster = Judge, masspoints = FALSE
  )$bins_plot
  ...
  p_obj +
    ...
    ylab("Best Model Probability Estimate and 95% UCB") +
    xlab("(Log) Input Message Token Length") +
    theme_bw() +
    ggtitle(main_title, 
            subtitle = "Input Token Length Heterogeneity")
}
...
iff_plot   <- iff_best[,"mistral_medium_3_1", drop = TRUE] #iff_pref[, model_plot, drop = TRUE]
...
p_clean <- p +
  ...
  annotate("text", x = -0.25, y = Inf, label = "Less Similar",
           hjust = 0.5, vjust = 1.2, size = 4, fontface = "italic") +
  annotate("text", x =  0.25, y = Inf, label = "More Similar",
           hjust = 0.5, vjust = 1.2, size = 4, fontface = "italic") +
...
opt.bestcost <- policy_tree(X, -centered_iff_cost_best, depth = 1) #4, split.step = 100)
...
results_bestcost = tibble(cost = policy_iff_cost_bestcost, 
                          ...
                          judge = Judge)
fixest::feols(c(cost, best) ~ 1, results_bestcost, vcov = "HC1") #cluster = ~judge)

centered_iff_best = apply(iff_best, 2, function(c) c - mean(c))
opt.best <- policy_tree(X, centered_iff_best, depth = 1) #2, split.step = 10)
...
results_best = tibble(cost = policy_iff_cost_best, 
                      ...
                      judge = Judge)
fixest::feols(c(cost, best) ~ 1, results_best, vcov = "HC1") #, cluster = ~judge)

centered_iff_cost = apply(iff_cost, 2, function(c) c - mean(c))
opt.cost <- policy_tree(X, -centered_iff_cost, depth = 1) #2, split.step = 10)
...
results_cost = tibble(cost = policy_iff_cost_cost, 
                      ...
                      judge = Judge)
fixest::feols(c(cost, best) ~ 1, results_cost, vcov = "HC1") #, cluster = ~judge)
...
bestcost_vs_best = tibble(cost = policy_iff_cost_bestcost - policy_iff_cost_best,
                          ...
                          judge = Judge)
fixest::feols(c(cost, best) ~ 1, bestcost_vs_best, vcov = "HC1") #, cluster = ~judge)
...
bestcost_vs_cost = tibble(cost = policy_iff_cost_bestcost - policy_iff_cost_cost,
                          ...
                          judge = Judge)
fixest::feols(c(cost, best) ~ 1, bestcost_vs_cost, vcov = "HC1") #, cluster = ~judge)
